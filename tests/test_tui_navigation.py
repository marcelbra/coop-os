...
@pytest.mark.parametrize(
    ...
    [
        (
            Nav("milestone", "milestone-2", "milestones"),
            ["milestone-1", "milestone-2", "milestone-3"],
            [
                Nav("milestone", "milestone-1", "milestones"),
                Nav("milestone", "milestone-2", "milestones"),
                Nav("milestone", "milestone-3", "milestones"),
            ],
            Nav("milestone", "milestone-2", "milestones"),
        ),
        (
...
def test_choose_same_section_neighbor(
    previous_nav: Nav, previous_ids: list[str], current_navs: list[Nav], expected: Nav | None
) -> None:
...
def test_enter_on_collapsed_branch_moves_to_first_child(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            ...
            tree.focus_nav(Nav("section", "", "tasks"))
            await pilot.pause()
            await _press_and_pause(pilot, "right")
            assert _cursor_nav(tree) == Nav("task", "task-1", "tasks")

            await _press_and_pause(pilot, "enter")
            assert _cursor_nav(tree) == Nav("task", "task-2", "tasks")
...
def test_right_on_leaf_enters_edit_mode(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            ...
            tree.focus_nav(Nav("task", "task-3", "tasks"))
            await pilot.pause()

            await _press_and_pause(pilot, "right")
            assert _cursor_nav(tree) == Nav("task", "task-3", "tasks")
            assert content.is_editing
            assert app.selected == Nav("task", "task-3", "tasks")
...
def test_right_on_expanded_branch_enters_edit_mode(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            ...
            assert _cursor_nav(tree) == Nav("task", "task-1", "tasks")
            assert content.is_editing
            assert app.selected == Nav("task", "task-1", "tasks")
...
def test_reload_preserves_section_header_focus(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            ...
            tree.focus_nav(Nav("section", "", "tasks"))
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("section", "", "tasks")
...
def test_reload_moves_filtered_milestone_to_next_visible_neighbor(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            tree = app.query_one(NavTree)
            tree.focus_nav(Nav("milestone", "milestone-2", "milestones"))
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("milestone", "milestone-2", "milestones")

            app.sm.milestone_filters = {"milestone-1", "milestone-3"}
            app.reload_state()
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("milestone", "milestone-3", "milestones")
...
def test_reload_moves_filtered_milestone_to_previous_visible_neighbor_when_no_next(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            tree = app.query_one(NavTree)
            tree.focus_nav(Nav("milestone", "milestone-3", "milestones"))
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("milestone", "milestone-3", "milestones")
...
def test_reload_moves_filtered_milestone_to_section_header_when_section_is_empty(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            ...
            app.reload_state()
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("section", "", "milestones")
...
def test_reload_moves_filtered_role_to_next_visible_neighbor(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            tree = app.query_one(NavTree)
            tree.focus_nav(Nav("role", "role-2", "roles"))
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("role", "role-2", "roles")

            app.sm.role_filters = {"role-1", "role-3"}
            app.reload_state()
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("role", "role-3", "roles")
...
def test_reload_moves_filtered_role_to_previous_visible_neighbor_when_no_next(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            tree = app.query_one(NavTree)
            tree.focus_nav(Nav("role", "role-3", "roles"))
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("role", "role-3", "roles")
...
def test_reload_moves_filtered_role_to_section_header_when_section_is_empty(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            ...
            app.reload_state()
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("section", "", "roles")
...
def test_reload_moves_filtered_task_to_next_visible_neighbor(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            tree = app.query_one(NavTree)
            tree.focus_nav(Nav("task", "task-2", "tasks"))
            await pilot.pause()
            assert _cursor_nav(tree) == Nav("task", "task-2", "tasks")
...
def test_next_nav_after_delete_for_top_level_task_prefers_next_sibling(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            await pilot.pause()
            next_nav = app.next_nav_after_delete(Nav("task", "task-1", "tasks"))
            assert next_nav == Nav("task", "task-3", "tasks")
...
def test_next_nav_after_delete_for_lone_subtask_returns_parent(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with _run_app(tmp_path) as (app, pilot):
            await pilot.pause()
            next_nav = app.next_nav_after_delete(Nav("task", "task-2", "tasks"))
            assert next_nav == Nav("task", "task-1", "tasks")
